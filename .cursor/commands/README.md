...
These are **Cursor commands** — POAIS is built for Cursor and uses Cursor commands, rules, skills, and subagents. Commands are the user-facing layer; each command delegates to a subagent and returns structured output. Product folders live under `products/<name>/`; use **`/status portfolio`** for roll-up.

## Available commands

| Command | Description |
|--------|-------------|
| `/setup-poais` | Conversational POAIS setup: agent asks product names, repo URL; runs init (portfolio layout); then offers what's next (CONTEXT, capture-idea, meeting notes, process input, or explore). |
| `/upgrade-poais` | Upgrade vendored poais-core: pull latest subtree and re-sync `.cursor/`. Agent runs the upgrade script from repo root (requires clean working tree). |
| `/process <path-to-input-file>` | Process messy input (chat, email, doc) into summary, extracted decisions/risks/scope, and minimal artifact updates. Tracks what was processed and when in a **Processed (POAIS)** block in the file so you can keep a running input and process chunks over time. |
...
/distill-meeting products/widget/INPUTS/YYYY-MM-DD-<slug>.md
...
Commands take a user-provided path (except `/release`, which is for the poais-core repo). **INPUTS** is the single source for unstructured raw input (notes, email, meeting jottings, future API/recordings). For meeting notes you can: (1) create a file in INPUTS, jot notes, run `/distill-meeting` to refine and catalogue to MEETINGS/; or (2) run **`/create-meeting-notes`** to create a MEETINGS file and capture live in Cursor. For ideas, run **`/capture-idea`** to create a file in IDEAS/. Then run `/process` on MEETINGS or IDEAS files to extract and update artifacts. You can keep a **running input file**, add to it over time, and run `/process` repeatedly; each run processes only new content and appends a row to the **Processed (POAIS)** block at the end of the file (see [process.md](process.md)). For `/process`, if the path does not exist, the command creates the parent directory and file, then uses pasted content if provided or asks the user to paste and rerun. The folder name need not be `product`; it is the default example only.

**Recommended first commands:** `/setup-poais` (for first-time setup), then `/process products/<name>/INPUTS/...`, `/distill-meeting products/<name>/INPUTS/...` (then `/process products/<name>/MEETINGS/...` on the catalogued file), `/create-meeting-notes`, `/capture-idea`, `/align products/<name>`, `/status products/<name>` (or `/status portfolio`).

## Short descriptions

- **setup-poais** — Conversational POAIS setup: one question at a time (product names, repo URL), confirm then run init (portfolio layout), then "what's next" menu (CONTEXT, capture-idea, meeting notes, process input, explore).
- **upgrade-poais** — Upgrade vendored poais-core: run poais-upgrade script from repo root (clean tree required); pull latest subtree, re-sync `.cursor/`.
- **process** — Turn one input file (or the next unprocessed chunk of a running file) into high-signal summary + proposed updates to DISCOVERY, PLAN, DECISIONS, RISKS, FEATURES, EXECUTION. Tracks processed sections in a Processed (POAIS) block in the file.
...
- **align** — Compare CONTEXT, PLAN, EXECUTION, DECISIONS (and optional PRD, ROADMAP/portfolio); report drift and suggest fixes.
- **status** — Generate team/stakeholder/exec status drafts and update STATUS.md. **`/status portfolio`** aggregates from all products and writes `portfolio/STATUS.md`.
...
| /upgrade-poais | [upgrade-poais.md](upgrade-poais.md) |
