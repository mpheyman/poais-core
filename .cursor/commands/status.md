...
- **product-folder** — Path to the product directory (e.g. `product`, `products/widget`) or the literal **`portfolio`**. For a single product, path must exist. For **`portfolio`**, aggregate status across all products (from POAIS_LOCK.json `products`) and write `portfolio/STATUS.md`; fail clearly if not a portfolio repo (no lock or no `products`).
...
2. **Single product:** Run the status-composer subagent on that product’s artifacts; update that product’s STATUS.md with the stakeholder update (and set “Week Of” if week-of provided).
