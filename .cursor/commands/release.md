...
1. **Read current state** — Read [VERSION](VERSION) and [CHANGELOG.md](CHANGELOG.md). Confirm there is an "Unreleased" section with at least one bullet. If Unreleased is empty, fail with a clear message (nothing to release).
...
- Do not release if "Unreleased" has no bullets; tell the user to add changes first.
